...
public class FiltroOrientacaoIdioma implements FiltroOrientacao<IdiomaOrientacao> {
	...
	public List<OrientacaoDto> aplicarFiltro(List<OrientacaoDto> listaObjeto) {
		if (idiomasOrientacoes.isEmpty()) {
			return listaObjeto;
		}

		return listaObjeto.stream().filter(this::filtrarPorIdioma).collect(Collectors.toList());
	}
	...
	public String pegarFiltrosEmTexto(IdiomaImplementacao idiomaImplementacao) {
		...
		for (IdiomaOrientacao idiomaFiltro : idiomasOrientacoes) {
			filtrosEmTexto.append(" " + numeroFiltroIdioma + " - "
					+ idiomaFiltro.getNomePorIdioma(idiomaImplementacao.obterIdiomaOrientacao()));
			numeroFiltroIdioma++;
		}
	...
	public void apagarFiltro(int index) {
		idiomasOrientacoes.remove(index);
	}
	...
	public String mostrarIdiomas(IdiomaOrientacao idiomaOrientacao) {
		...
		idiomasOrientacoes.stream()
				.forEach(i -> formatadoMostrarIdiomas.append(i.getNomePorIdioma(idiomaOrientacao)).append(", "));
