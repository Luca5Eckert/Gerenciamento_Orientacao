...
public class ValidarUsuario implements Validacao, Runnable {
	...
	private boolean comecaComEspaco() throws CadastroException {
		boolean usuarioValido = usuarioParaValidar != null && !usuarioParaValidar.trim().isEmpty()
				&& !usuarioParaValidar.startsWith(" ");
	...
	private boolean excedeLimiteCaracteres() {
		...
		if (!usuarioValido) {
			throw new CadastroUsuarioJaExistenteException(
					idiomaImplementacao.pegarMensagemUsuarioInvalidoLimiteDeCaracters());
		}
		return true;
	}
