...
public enum IdiomaOrientacao implements Filtro<IdiomaOrientacao> {
	...
	public static String listaIdiomasFormatado(IdiomaImplementacao idiomaImplementacao) {
		...
		for (IdiomaOrientacao idioma : values()) {
			idiomasFormatados.append(" ").append(numeroIdioma).append("- ")
					.append(idioma.getNomePorIdioma(idiomaImplementacao.obterIdiomaOrientacao())).append("\n");
			numeroIdioma++;
		}
	...
	public static IdiomaOrientacao valueOfNome(String nome) {
		for (IdiomaOrientacao idioma : values()) {
			if (idioma.idiomaPortugues.equalsIgnoreCase(nome) || idioma.idiomaIngles.equalsIgnoreCase(nome)
					|| idioma.idiomaAlemao.equalsIgnoreCase(nome) || idioma.idiomaEspanhol.equalsIgnoreCase(nome)) {
				return idioma;
			}
		}
	...
	public List<String> pegarValoresSegundoIdioma(IdiomaOrientacao idiomaOrientacao) {
		return Arrays.stream(values()).map(i -> i.getNomePorIdioma(idiomaOrientacao)).toList();
	}
